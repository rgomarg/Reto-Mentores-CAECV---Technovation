...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-[#C1C69A] flex items-center justify-between px-4 py-3 md:px-8 shadow-sm text-[#1C201C] relative z-20">
      ...
      {/* Avatar circular -> Al hacer clic navega a la nueva página de Usuario */}
      <div 
        className={`w-10 h-10 sm:w-12 sm:h-12 bg-[#DE6D5C] rounded-full flex items-center justify-center text-white font-bold text-xl cursor-pointer shadow-sm hover:scale-105 transition-all ${
          isUsuarioActive ? 'ring-4 ring-white shadow-md' : 'hover:ring-2 hover:ring-black/20'
        }`}
        onClick={() => navigate(`/usuario/${loggedUserId}`)}
        title={`Ver perfil de usuario (${loggedUserName})`}
      >
        {avatarLetter}
      </div>
