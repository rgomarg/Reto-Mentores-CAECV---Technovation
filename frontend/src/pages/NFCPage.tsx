...
export default function NFCPage() {
  ...
  useEffect(() => {
    // Fetch para saber qué cromo viene en el sobre
  ...
  const abrirSobre = () => {
    ...
    setEstado('abriendo');
    // Simulamos el tiempo de animación antes de revelar el cromo
    setTimeout(() => {
      setEstado('revelado');
    }, 1500);
  };
  ...
  const guardarCromo = () => {
    ...
    .then(res => {
      if (res.ok) {
        // Redirigir al álbum pasando el id del nuevo cromo para destacarlo
        navigate(`/dashboard/${idUsuario}/album`, { state: { nuevoCromoDestacado: Number(idCromo) } });
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#a8e6a3] to-[#80d07b] flex flex-col items-center justify-center p-4 relative font-sans overflow-hidden">
      ...
      {estado === 'sobre' && (
        ...
        >
          ...
          <div className="absolute bg-white/80 text-black px-4 py-2 rounded-full font-bold shadow-lg mt-40">
            ¡Toca para abrir!
          </div>
        </div>
      )}
      ...
      {estado === 'revelado' && (
        <div className="flex flex-col items-center justify-center animate-[zoomIn_0.5s_ease-out]">
          <h2 className="text-2xl font-bold mb-4 text-emerald-900">¡Has conseguido a {cromo.nombre}!</h2>
